...
// TO DO: complete this function following instructions
{
	...
	if (pt->frame == NULL) {
		printf("Failed to allocate an dynamic array!\n");
		return;
	}
...
// load page table from a file
{
	...
	for (int i=0; i<pt->no_page; i++)
		fscanf(fp, "%d", &pt->frame[i]);

	if(PageTable_Save(pt, filename) == FALSE) {
		printf("failed to load a page table!\n");
		return FALSE;
	}
