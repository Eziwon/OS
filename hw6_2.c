...
int main(int argc, char *argv[])
{
	...
	for (int i=0; i<2; i++) {
		param[i].p_sb = &sb;
		param[i].p_cont = &thread_cont;
		param[i].p_mutex = &mutex;
		param[i].p_empty = &empty;
		param[i].p_full = &full;
	}
	...
	pthread_create(&producer, NULL, Producer, &param[0]);
	pthread_create(&consumer, NULL, Consumer, &param[1]);
...
void* Producer(void *arg)
{
	...
	while(*param->p_cont){
		...
		strcpy(mesg, message_array[(rand() % no_message)]);
